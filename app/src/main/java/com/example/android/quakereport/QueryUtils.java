    ...
    public static ArrayList<Earthquake> extractEarthquakes(String jsonResponse) {
        ...
        try {
            ...
            for (int i = 0; i < featuresArray.length(); i++) {
                JSONObject earthquakeJSON = featuresArray.optJSONObject(i);
                // Isolate "properties" object
                JSONObject propertiesJSON = earthquakeJSON.optJSONObject("properties");
                // Extract magnitude, place and time data
                double magnitude = propertiesJSON.optDouble("mag");
                String location = propertiesJSON.optString("place");
                long date = propertiesJSON.optLong("time");
                String url = propertiesJSON.optString("url");
                // Add new earthquake to the list
